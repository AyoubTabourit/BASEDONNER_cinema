...
export class CinemaService {
  private Checkout = 'http://localhost:8083/payerTickets';
  imageUrl = 'http://localhost:8083/imageFilm/';

  constructor(private http: HttpClient) {}

  getCinemas(ville: getCinemasAPI): Observable<Cinema[]> {
    return this.http
      .get<getCinemas>(ville._links.cinemas.href)
      .pipe(map((data) => data._embedded.cinemas));
  }
  getSallesByCinema(cinema: getSallesAPI): Observable<Salle[]> {
    return this.http
      .get<getSalles>(cinema._links.salle.href)
      .pipe(map((data) => data._embedded.salles));
  }
